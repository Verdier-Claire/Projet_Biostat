...
Puisque nous avons un très grand nombre de type de mort, on décide de se contrer sur un seul type : les morts par cancer. De plus comme dis plus haut, le mode de l'âge est très élévé, on décide donc de se réduire aux individus entre 65 et 79 ans.
...
Voici plusieurs graphs qui résument notre taux de décès en fonction du sexe et de la période dans notre jeu
```{r}
...
g2 <- ggplot(mort)+ 
  geom_point(aes(x=periode, y=nb.Cancer,  color=age))+
  ggtitle("Nombre de mort par cancer en fonction de l'âge")


g3 <- ggplot(mort)+ 
  geom_bar(aes(x=age, y=nb.Cancer,  fill=sex),
           stat="identity", 
